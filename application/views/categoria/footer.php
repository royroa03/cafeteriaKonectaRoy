  <!-- jQuery -->
    ...
    <script>
        ...
        $('#mdlEditarCategoria').on('show.bs.modal', function (event) {
          var button = $(event.relatedTarget);
          var id_categoria = button.data('idcategoria');
          var modal = $(this);
          $('.loading').show();
          $('#id_categoria').val(id_categoria);
          console.log(id_categoria);
          params = {csrfsn: csrfsn, id_categoria: id_categoria }
          $.post(base_url+'ajax/getcategoria/', params, function(data){
              if(data.status > 0){
                  console.log(data);
                  $('#edit_nombre').val(data.categorias.nombre);
        ...
        $('#mdlEditarCategoria').on('hidden.bs.modal', function (e) {
          var modal = $(this);
          $('#nombre').val('');
        });
          ...
          swal(
              {
                  ...
                  text: "Eliminando Categoría Seleccionada",
                  type: "warning",
              ...
              function (isConfirm) {
                  if (isConfirm) {
                      $.get(base_url + "categoria/eliminar/"+id_categoria, function(data){
                          if(data.status==1){
