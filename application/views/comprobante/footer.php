  <!-- jQuery -->
    ...
    <script>
        ...
        $('#mdlEditarComprobante').on('show.bs.modal', function (event) {
          var button = $(event.relatedTarget);
          var id_comprobante = button.data('idcomprobante');
          var modal = $(this);
          $('.loading').show();
          $('#id_comprobante').val(id_comprobante);
          console.log(id_comprobante);
          params = {csrfsn: csrfsn, id_comprobante: id_comprobante }
          $.post(base_url+'ajax/getcomprobante/', params, function(data){
              if(data.status > 0){
                  console.log(data);
                  $('#edit_codigo').val(data.comprobantes.codigo);
                  $('#edit_descripcion').val(data.comprobantes.descripcion);
        ...
        $('#mdlEditarComprobante').on('hidden.bs.modal', function (e) {
          var modal = $(this);
          $('#codigo').val('');
          $('#descripcion').val('');
        });

        function eliminarComprobante(id_comprobante){
          swal(
              {
                  title: "¿Está seguro que desea ELIMINAR el COMPROBANTE?",
                  text: "Eliminando Comprobante Seleccionado",
                  type: "warning",
              ...
              function (isConfirm) {
                  if (isConfirm) {
                      $.get(base_url + "comprobante/eliminar/"+id_comprobante, function(data){
                          if(data.status==1){
