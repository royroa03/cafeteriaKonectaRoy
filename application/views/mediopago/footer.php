  <!-- jQuery -->
    ...
    <script>
        ...
        $('#mdlEditarMedioPago').on('show.bs.modal', function (event) {
          var button = $(event.relatedTarget);
          var id_mediopago = button.data('idmediopago');
          var modal = $(this);
          $('.loading').show();
          $('#id_mediopago').val(id_mediopago);
          console.log(id_mediopago);
          params = {csrfsn: csrfsn, id_mediopago: id_mediopago }
          $.post(base_url+'ajax/getmediopago/', params, function(data){
              if(data.status > 0){
                  console.log(data);
                  $('#edit_nombre').val(data.mediospagos.nombre);
        ...
        $('#mdlEditarMedioPago').on('hidden.bs.modal', function (e) {
          var modal = $(this);
          $('#nombre').val('');
        });
        ...
        function eliminarMedioPago(id_mediopago){
          swal(
              {
                  title: "¿Está seguro que desea ELIMINAR el MEDIO DE PAGO?",
                  text: "Eliminando Medio de Pago Seleccionado",
                  type: "warning",
              ...
              function (isConfirm) {
                  if (isConfirm) {
                      $.get(base_url + "mediopago/eliminar/"+id_mediopago, function(data){
                          if(data.status==1){
