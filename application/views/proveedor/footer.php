  <!-- jQuery -->
    ...
    <script>
        ...
        $('#mdlEditarProveedor').on('show.bs.modal', function (event) {
          var button = $(event.relatedTarget);
          var id_proveedor = button.data('idproveedor');
          var modal = $(this);
          $('.loading').show();
          $('#id_proveedor').val(id_proveedor);
          console.log(id_proveedor);
          params = {csrfsn: csrfsn, id_proveedor: id_proveedor }
          $.post(base_url+'ajax/getproveedor/', params, function(data){
              if(data.status > 0){
                  console.log(data);
                  $('#edit_tipo_doc').val(data.proveedores.tipo_doc).trigger('chosen:updated');
                  $('#edit_nro_doc').val(data.proveedores.nro_doc);
                  $('#edit_razon_social').val(data.proveedores.razon_social);
                  $('#edit_telefono1').val(data.proveedores.telefono1);
                  $('#edit_telefono2').val(data.proveedores.telefono2);
                  $('#edit_direccion').val(data.proveedores.direccion);
                  $('#edit_referencia').val(data.proveedores.referencia);
        ...
        $('#mdlEditarProveedor').on('hidden.bs.modal', function (e) {
          var modal = $(this);
